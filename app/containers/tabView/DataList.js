...
import { View, Text, StyleSheet, Image,TouchableOpacity,ScrollView,Modal,
...
import { SCREEN_WIDTH,little_font_size,SCREEN_HEIGHT, little_font_size2, WINDOW_HEIGHT, } from '../../config/globalsize';
/*
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={[{width:SCREEN_WIDTH,flex:1,},]}>
                            <View style={[{width:SCREEN_WIDTH,height:33,flexDirection:'row',},]}>
                                    <View style={[styles.oneLabel,styles.myBorderBottom,]}>
                                            <View style={[{justifyContent:'center',alignItems:'center',},]}>
                                                <Text style={[{fontSize:little_font_size,}]}>监测点</Text>
                                            </View>
                                           }
                                            <TouchableOpacity style={{marginLeft:4,width:32,height:24,justifyContent:'center'}} onPress={()=>{this.props.dispatch(createAction('datapreview/setState')({groupSelectorVisible:true}));}}>
                                                <Icon name={'md-arrow-dropdown'} size={24} style={[{color:globalcolor.titleBlue}]} />   
                                                {/*<Image source={require('../../../../images/ic_no_minitor.png')} style={[{width:24,height:24,}]}/>*/}
                                            </TouchableOpacity>
                                    </View>
                                <ScrollView ref={ref => this.titleScrollView = ref} style={[{width:SCREEN_WIDTH*2/3,height:33},]} horizontal={true}
                                showsHorizontalScrollIndicator={false} scrollEnabled={false}> 
                                    <LabelHeadView />
                                </ScrollView>
                            </View>

                        <ScrollView style={[{height:80,width:SCREEN_WIDTH,}]} 
                            refreshControl={
                                <RefreshControl
                            ...
                            onMomentumScrollEnd={this._contentViewScroll} showsVerticalScrollIndicator={false}>
                             ...
                             this.state.cityNameLst.map(item =>{
                                  ...
                                  return(<TouchableOpacity key = {item.key} onPress={()=>{console.log('onPress');}}
                                            style={[{width:SCREEN_WIDTH/3,
                                                        height:60,
                                                        justifyContent:'center',
                                                        alignItems:'flex-start',paddingLeft:5,backgroundColor : 'white'},styles.myBorderBottom,]}>
                                    ...
                                    <View style={{flexDirection:'row',alignItems:'center'}}>
                                        <View style={[{width:little_font_size2,height:little_font_size2,borderRadius:5,
                                            ...
                                            marginRight:5,}]}></View>
                                            <Text style={[{fontSize:little_font_size,width:SCREEN_WIDTH/3 - little_font_size2}]}>{item.entName}</Text>
                                    </View>  
                                    
                                    {item.entName === "大唐集团" ?  <View style = {{height : 15 ,backgroundColor:'#ffe4e1' ,alignItems : 'center',justifyContent:'center',flexDirection : 'row',marginTop : 1,borderWidth : 1,borderColor:'#ff4500',borderRadius : 3}} >
                                   
                                    <Text style={[{fontSize:10,alignItems : 'center',justifyContent:'center',color : '#ff4500'}]}>
                                     维护
                                    ...
                                    <ScrollView 
                                        ...
                                        showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false} horizontal={true} style={[styles.HorizontalList,{height:1000,}]}>
                                      
                                        <View style={[{height:this.state.cityNameLst?33+this.state.cityNameLst.length*40:33,
                                          width:this.state.cityNameLst?SCREEN_WIDTH/3*this.state.cityNameLst.length:0,backgroundColor:'white'}]}>
                              
                              {
                                  this.state.cityNameLst.map((item)=>{
                                     
                                      return (<TouchableOpacity key={item.key} style={[{height:60,width:SCREEN_WIDTH/3*this.state.cityNameLst.length,flexDirection:'row',},styles.myBorderBottom,]}>
                                        <Text key={item.key} style={[{width:SCREEN_WIDTH/3,height:39,textAlign:'center',lineHeight:40,color:item.Colors&&item.Colors[_key]?item.Colors[_key]:'black'}]}>
                                        {item.monitorTime}
                                        </Text>
                                              {
                                                ...
                                                item.MonitoringDatasi.PollutantDatas.map((zcc) =>
                                                {
                                                  return(
                                                    ...
                                                    <Text key={zcc.PollutantCode} style={[{width:SCREEN_WIDTH/3,height:39,textAlign:'center',lineHeight:40,color:item.Colors&&item.Colors[_key]?item.Colors[_key]:'black'}]}>
                                                    {zcc.Concentration}
                                                    </Text>);
                                                })
