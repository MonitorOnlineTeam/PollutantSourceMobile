    ...
    render() {
        return (
            <View style={[styles.container,{flexDirection:'row',borderBottomColor:globalcolor.borderLightGreyColor,
                        borderBottomWidth:1,},{width:defaultPollutantCodes.length*SCREEN_WIDTH/3,height:32,},]}>
                {
                    defaultPollutantCodes.map(item=>{
                    return (<View  key = {item.Value} style={[styles.oneLabel,]}>
                        <View style={[{height:28,width:1,backgroundColor:globalcolor.borderGreyColor,}]}></View>
                        <View style={[{flex:1,justifyContent:'center',alignItems:'center',},]}>
                            <Text style={[{fontSize:little_font_size,}]}>{item.Name}</Text>
